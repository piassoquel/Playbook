...
function detail(id){
  ...
  app.querySelectorAll('[data-alt-size]').forEach(button=>button.onclick=()=>{
    const text=button.dataset.altDirection==='up'
      ?'Sizing up can add float in powder and more stability at speed, at the cost of a little quickness.'
      :'Sizing down can make the board lighter and quicker to turn, at the cost of a little float and stability.';
    openSetupSheet('ALTERNATIVE SIZE',button.dataset.altSize,text);
  });
